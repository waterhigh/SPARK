...
class ResNetWithTransformer(nn.Module):
    ...
    def forward(self, video_frames, return_msm=False):
        """
        return_msm (bool): 是否计算 Masked Spike Modeling Loss (仅训练时开启)
        """
        B, T, C, H, W = video_frames.shape
        
        # --- 1. Feature Extractor & ResNet ---
        processed_blocks = []
        for b in range(B):
        ...
        # 分离输出
        global_feature = x_out[:, 0]     # CLS Token
        local_features = x_out[:, 1:]    # Frame Tokens [B, T_new, D]
        
        # ================= [优化 3] 计算 MSM Loss =================
        if self.training and return_msm and bool_mask is not None:
            # 1. 预测特征：将 Transformer 输出的 Frame Tokens 映射回原始空间
            pred_features = self.decoder_head(local_features)
            
            # 2. 只计算被 Mask 位置的 Loss
            pred_masked = pred_features[bool_mask] # [N_masked, D]
            
            # 3. MSE Loss: 预测特征 vs 原始 ResNet 特征
            # detach target 是为了防止 MSM 任务去“改变” ResNet 的特征提取方式来“作弊”
            # 强迫 Transformer 学习上下文关系，而不是让 Backbone 变弱
